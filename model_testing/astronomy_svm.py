...
def add_color_indices(df):
    ...
    df_with_features["u-g"] = df_with_features["u"] - df_with_features["g"]
    df_with_features["g-r"] = df_with_features["g"] - df_with_features["r"]
    df_with_features["r-i"] = df_with_features["r"] - df_with_features["i"]
    df_with_features["i-z"] = df_with_features["i"] - df_with_features["z"]
    df_with_features["u-r"] = (
        df_with_features["u"] - df_with_features["r"]
    )  # Wider color baseline
    df_with_features["g-i"] = (
        df_with_features["g"] - df_with_features["i"]
    )  # Wider color baseline
...
balanced_with_features = add_color_indices(balanced_train_df)
...
print(balanced_with_features.info())

# %% Creating test split data set with sample size for faster training

X = balanced_with_features.drop(columns=["class", "class_labels"])
y = balanced_with_features["class"]

# Create an initial feature selection split
X_train, X_test, y_train, y_test = msel.train_test_split(
    X, y, test_size=0.40, random_state=42, stratify=y
)
# %% Pipeline setup for kernel svm without pca

svm_model = svm.SVC(random_state=42)  # kernel='rbf' er default
scaler = prep.MinMaxScaler()

pipe_svm = pipe.Pipeline([("scaler", scaler), ("svm", svm_model)])
...
search = msel.GridSearchCV(
    pipe_svm, param_grid_svm, cv=3, n_jobs=-1, scoring="f1_macro"
)
...
final_svm = svm.SVC(
    kernel=search.best_kernel_["svm_model__kernel"],
    C=search.best_params_["svm_model__C"],
    degree=search.best_params_["svm_model__degree"],
    gamma=search.best_params_["svm_model__gamma"],
    random_state=42,
...
final_pipeline = pipe.Pipeline([("random_forest", final_svm)])
...
test_full_features_reduced = test_full_features.drop(columns=["i", "z"])

# %%

# The pipeline handles scaling internally
y_test = final_pipeline.predict(test_full_features_reduced)
...
filename = "svm_submission__features_est.csv"
