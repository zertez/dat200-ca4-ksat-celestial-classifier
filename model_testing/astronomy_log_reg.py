...
train_df['class'] = train_df['class'].map(
    class_mapping)  # Replace with numeric codes
...
print("There are a lot of observations in the original dataset, we therefore extract a sample of 5000 to visualize our data. This still represents our data well, but its less taxing to compute and plot.")

sample_size = 5000
plot_df = train_df.columns.drop(["class"])
plot_df_sample = train_df.sample(n=sample_size, random_state=42)

print(plot_df_sample['class_labels'].value_counts(
    normalize=True).mul(100).round(2).astype(str) + ' %')
...
for i, feature in enumerate(features):
    sns.boxplot(x="class_labels", y=feature, data=plot_df_sample, hue="class_labels",
...
axes = axes.flatten()
for i, feature in enumerate(features):
    sns.histplot(data=plot_df_sample, x=feature, hue="class_labels",
                 palette="Set1", ax=axes[i], alpha=0.4, multiple="layer", kde=True)
...
# %% Correlation Matrix
samples = [plot_df_sample[plot_df_sample['class_labels'] == cls][features]
           for cls in classes]
...
for i, (df, title) in enumerate(zip(samples, classes)):
    ...
    sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', fmt='.2f',
                ax=axes[i], square=True, vmin=-1.0, vmax=1.0)
...
galaxies_downsampled = util.resample(
    galaxies, n_samples=target_size, random_state=42)
...
X = balanced_train_df.drop(columns=["class", "class_labels"])
y = balanced_train_df["class"]

# Create an initial feßßature selection split
X_train, X_test, y_train, y_test = msel.train_test_split(
    X, y, test_size=0.40, random_state=42, stratify=y
)
# %% Pipeline setup for logreg with pca
pca = decomp.PCA(n_components=10, random_state=42,
                 svd_solver='full')  # Using a fixed number
logreg = lm.LogisticRegression(random_state=42)
scaler = prep.StandardScaler()

pipe_with_pca = pipe.Pipeline([("scaler", scaler), ("pca", pca),
                              ("logreg", logreg)
                               ])
...
param_grid = {
    ...
    'logreg__max_iter': [100, 200, 1000],
    'logreg__tol': [0.0001, 0.001, 0.01],  # Try a tighter tolerance
    'logreg__penalty': ['l2'],  # Stick with l2 for numerical stability
    'logreg__solver': ['saga', 'lbfgs'],  # Both work with l2
    # Wider range of regularization strengths
    'logreg__C': [0.1, 1, 10, 50, 100],
    # For handling class imbalance if needed
    'logreg__class_weight': [None, 'balanced'],
}

search = msel.GridSearchCV(pipe_with_pca, param_grid,
                           cv=3, n_jobs=4, scoring="f1_macro")
...
print(f"Best parameters from sample training:")
...
param_grid = {
    ...
    'logreg__solver': ["sag", "saga", "newton_cg"],
...
final_logreg = ens.RandomForestClassifier(
    max_iter=search.best_params_['logreg__max_iter'],
    penalty=search.best_params_['logreg__penalty'],
    solver=search.best_params_['logreg__solver'],
    multi_class=search.best_params_['logreg__multi_class'],
...
final_pipeline = pipe.Pipeline([("scaler", scaler),
                                ("logreg", final_logreg)
                                ])
